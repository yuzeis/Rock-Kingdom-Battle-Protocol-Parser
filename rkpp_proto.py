...
def parse_record(packet: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if packet["direction"] == "s2c" and len(body) >= 10 and body[4:6] == b"\x55\xaa":
        ...
        special = parse_special_payload(op, payload)
        return {
            **common, "opcode": op, "opcode_hex": f"0x{op:04X}", "subtype": sub,
            "payload_len": len(payload), "payload_trailer_len": trailer_len,
            "payload_format": special[0] if special else "protobuf",
            "special_payload": special[1] if special else None,
            "root": (
                {"fields": [], "consumed": len(payload), "clean": True}
                if special else
                (parse_proto_message(payload) if payload else {"fields": [], "consumed": 0, "clean": True})
            ),
        }
    ...
    if packet["direction"] == "c2s" and len(body) >= 14 and body[8:10] == b"\x39\x63":
        ...
        raw_op = int.from_bytes(body[4:8], "big")
        op, normalized = normalize_c2s_opcode(raw_op)
        ...
        return {
            **common, "opcode": op, "opcode_hex": f"0x{op:04X}",
            "raw_opcode": raw_op, "raw_opcode_hex": f"0x{raw_op:08X}",
